...
class PoseNMS:
    def __init__(self):
        ...
        self.delta2 = 2.65
        self.gamma = 22.48
        self.scoreThreds = 0.3
        self.matchThreds = 5
        self.areaThres = 0  # 40 * 40.5
        self.alpha = 0.1
        self.ori_pose_preds = []
        self.ori_pose_scores = []
        self.ref_dists = []

    def pose_nms(self, bboxes, bbox_scores, pose_preds, pose_scores):
        """
        ...
        self.ori_pose_preds = pose_preds.clone()
        self.ori_pose_scores = pose_scores.clone()
        ...
        widths = xmax - xmin
        heights = ymax - ymin
        self.ref_dists = self.alpha * np.maximum(widths, heights)
        ...
        while human_scores.shape[0] != 0:
            ...
            ref_dist = self.ref_dists[human_ids[pick_id]]
            simi = self.get_parametric_distance(pick_id, pose_preds, pose_scores, ref_dist)
            num_match_keypoints = self.PCK_match(pose_preds[pick_id], pose_preds, ref_dist)

            # Delete humans who have more than self.matchThreds keypoints overlap and high similarity
            delete_ids = torch.from_numpy(np.arange(human_scores.shape[0]))[
                (simi > self.gamma) | (num_match_keypoints >= self.matchThreds)]
        ...
        preds_pick = self.ori_pose_preds[pick]
        scores_pick = self.ori_pose_scores[pick]
        ...
        for j in range(len(pick)):
            ...
            if max_score < self.scoreThreds:
                continue

            # Merge poses
            merge_id = merge_ids[j]
            merge_pose, merge_score = self.p_merge_fast(
                preds_pick[j], self.ori_pose_preds[merge_id], self.ori_pose_scores[merge_id], self.ref_dists[pick[j]])
            ...
            if 1.5 ** 2 * (xmax - xmin) * (ymax - ymin) < self.areaThres:
                continue
    ...
    def filter_result(self, args):
        ...
        # Merge poses
        merge_pose, merge_score = self.p_merge_fast(
            pred_pick, self.ori_pose_preds[merge_id], self.ori_pose_scores[merge_id], self.ref_dists[pick])
    ...
    def p_merge(self, ref_pose, cluster_preds, cluster_scores, ref_dist):
        """
    ...
    def p_merge_fast(self, ref_pose, cluster_preds, cluster_scores, ref_dist):
        """
        Score-weighted pose merging
        INPUT:
            ref_pose:       reference pose          -- [17, 2]
    ...
    def get_parametric_distance(self, i, all_preds, keypoint_scores, ref_dist):
        ...
        score_dists[mask] = torch.tanh(pred_scores[mask] / self.delta1) * \
                            torch.tanh(keypoint_scores[mask] / self.delta1)

        point_dist = torch.exp((-1) * dist / self.delta2)
        final_dist = torch.sum(score_dists, dim=1) + self.mu * torch.sum(point_dist, dim=1)

        return final_dist

    def PCK_match(self, pick_pred, all_preds, ref_dist):
        dist = torch.sqrt(torch.sum(
